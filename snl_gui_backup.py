...
SCREEN_HEIGHT = 850  # Increased for more space
BOARD_SIZE = 520  # Slightly decreased to give more space for info panel
...
class SnakesAndLaddersGUI:
    ...
    def get_safe_game_copy(self):
        ...
        for player in self.game.players:
            if isinstance(player, HumanGUIPlayer):
                ...
                new_player.skip_turn = player.skip_turn
                safe_game.add_player(new_player)
            elif isinstance(player, AIPlayer):
                # Create a new AI player
                new_player = AIPlayer(player.name, player.difficulty)
                new_player.position = player.position
                new_player.tokens = player.tokens
                new_player.skip_turn = player.skip_turn
                safe_game.add_player(new_player)
    ...
    def draw_human_controls(self):
        ...
        elif self.waiting_for_token_decision:
            # Draw token usage decision buttons
            snake_head = self.current_player.position
            tokens_required = self.game.snake_sizes[snake_head]
            
            title = self.font.render(f"Use {tokens_required} tokens to neutralize the snake?", True, BLACK)
            self.screen.blit(title, (BOARD_OFFSET_X + 20, control_y + 15))
            
            # Yes button
            yes_rect = pygame.Rect(BOARD_OFFSET_X + BOARD_SIZE//4 - 50,
                                 control_y + 50, 100, 50)
            pygame.draw.rect(self.screen, (100, 200, 100), yes_rect, border_radius=8)
            pygame.draw.rect(self.screen, BLACK, yes_rect, 2, border_radius=8)
            yes_text = self.font.render("Yes", True, BLACK)
            text_rect = yes_text.get_rect(center=yes_rect.center)
            self.screen.blit(yes_text, text_rect)
            
            # No button
            no_rect = pygame.Rect(BOARD_OFFSET_X + 3*BOARD_SIZE//4 - 50,
                                control_y + 50, 100, 50)
            pygame.draw.rect(self.screen, (200, 100, 100), no_rect, border_radius=8)
            pygame.draw.rect(self.screen, BLACK, no_rect, 2, border_radius=8)
            no_text = self.font.render("No", True, BLACK)
            text_rect = no_text.get_rect(center=no_rect.center)
            self.screen.blit(no_text, text_rect)
    ...
    def handle_prediction_click(self, pos):
        """Handle clicks on prediction buttons and confirm button."""
        if not self.waiting_for_prediction:
            return None
        
        control_y = BOARD_SIZE + BOARD_OFFSET_Y + 20
        
        # Check prediction buttons (matching the draw locations exactly)
        for i in range(1, 7):
            button_x = BOARD_OFFSET_X + (i-1) * 80 + 30  # Exact same calculation as in draw method
            button_rect = pygame.Rect(button_x, control_y + 50, 60, 40)  # Same dimensions
            
            if button_rect.collidepoint(pos):
                self.human_prediction = i
                print(f"Selected prediction: {i}")  # Debug print
                return None
        
        # Check confirm button using stored rectangle when available
        if self.human_prediction is not None:
            # Use the same rectangle that was drawn
            if hasattr(self, 'confirm_button_rect'):
                confirm_rect = self.confirm_button_rect
            else:
                # Fallback in case not drawn yet
                confirm_rect = pygame.Rect(BOARD_OFFSET_X + BOARD_SIZE//2 - 100,
                                      control_y + 110, 200, 60)
            
            if confirm_rect.collidepoint(pos):
                print(f"Confirming prediction: {self.human_prediction}")
                print(f"Button rect: {confirm_rect}, Click at: {pos}")  # Debug exact position
                
                # Visual feedback - flash the button
                orig_screen = self.screen.copy()
                flash_rect = pygame.Rect(confirm_rect)
                pygame.draw.rect(self.screen, (255, 255, 150), flash_rect, border_radius=8)
                pygame.draw.rect(self.screen, BLACK, flash_rect, 2, border_radius=8)
                pygame.display.flip()
                pygame.time.delay(100)  # Brief flash
                
                # Process the confirmation
                self.waiting_for_prediction = False
                self.game.make_prediction(self.current_player.name, self.human_prediction)
                self.add_message(f"{self.current_player.name} predicts: {self.human_prediction}")
                ...
                # If current player predicted correctly, wait for reward choice
                ...
                print(f"Click at {pos} missed confirm button at {confirm_rect}")
    ...
    def handle_token_decision_click(self, pos):
        """Handle player's decision to use tokens or slide down the snake."""
        if not self.waiting_for_token_decision:
            return None
            
        control_y = BOARD_SIZE + BOARD_OFFSET_Y + 20
        
        # Debug log
        print(f"Token decision click at {pos}")
        
        # Check yes button
        yes_rect = pygame.Rect(BOARD_OFFSET_X + BOARD_SIZE//4 - 50,
                             control_y + 50, 100, 50)
        if yes_rect.collidepoint(pos):
            print("Yes button clicked (use tokens)")
            self.waiting_for_token_decision = False
            
            try:
                # Use tokens to neutralize snake
                snake_head = self.current_player.position
                tokens_required = self.game.snake_sizes[snake_head]
                self.current_player.tokens -= tokens_required
                self.add_message(f"{self.current_player.name} used {tokens_required} tokens to neutralize the snake!")
                
                # Move to next player's turn without executing potentially problematic callbacks
                print("Skipping callbacks and moving to next player's turn")
                self.after_animation_callback = None  # Clear any callbacks
                self.animation_in_progress = False  # Ensure no animations in progress
                self.game.current_player_idx = (self.game.current_player_idx + 1) % len(self.game.players)
                return False  # Continue game
            except Exception as e:
                print(f"Error in token use handler: {e}")
                # Recover from error and continue game
                self.game.current_player_idx = (self.game.current_player_idx + 1) % len(self.game.players)
                return False
        ...
        if no_rect.collidepoint(pos):
            print("No button clicked (slide down snake)")
            self.waiting_for_token_decision = False
            ...
            try:
                ...
                self.add_message(f"{self.current_player.name} slid down to position {snake_tail}!")
                
                # Clear any problematic callbacks
                self.after_animation_callback = None
                
                # Update player position directly without animation if there are issues
                self.current_player.position = snake_tail
                
                # Set a safe callback - just move to next player's turn
                self.game.current_player_idx = (self.game.current_player_idx + 1) % len(self.game.players)
                
                return False  # Continue game without animation
            except Exception as e:
                print(f"Error in snake slide handler: {e}")
                # Recover from error and continue game
                if snake_head in self.game.snakes:
                    self.current_player.position = self.game.snakes[snake_head]
                self.game.current_player_idx = (self.game.current_player_idx + 1) % len(self.game.players)
                return False
    ...
    def handle_snake_encounter(self, player):
        ...
        try:
            ...
            # For human player, let them decide
            if isinstance(player, HumanGUIPlayer) and player == self.current_player:
                ...
                self.waiting_for_token_decision = True
                return False
            ...
            if player.tokens >= tokens_required:
                ...
                if use_tokens:
                    ...
                    if self.after_animation_callback:
                        # Store the callback locally before clearing it to avoid issues
    ...
    def handle_event(self, event):
        """Handle pygame events, particularly mouse clicks for user interactions."""
        if event.type == pygame.QUIT:
            return False  # Signal to exit
        
        # Handle mouse clicks
        if event.type == pygame.MOUSEBUTTONDOWN:
            print(f"Mouse click at {event.pos}")  # Debug
            
            # Handle game-specific clicks
            if self.waiting_for_prediction:
                return self.handle_prediction_click(event.pos)
            
            elif self.waiting_for_reward_choice:
                choice = self.handle_reward_choice_click(event.pos)
                if choice is not None:
                    return choice
            
            elif self.waiting_for_token_decision:
                decision = self.handle_token_decision_click(event.pos)
    ...
    def update_animations(self):
        ...
        if elapsed >= self.animation_duration:
            ...
            if hasattr(self, 'after_animation_callback') and self.after_animation_callback:
                result = self.after_animation_callback()
                self.after_animation_callback = None
                return result
    ...
    def handle_opponent_predictions(self, dice_roll, current_player):
        ...
        correct_opponents = [p for p in self.game.players 
                          if p != current_player and self.game.predictions.get(p.name, 0) == dice_roll]
        ...
        if len(correct_opponents) >= 1:
            self.add_message(f"{correct_opponents[0].name} predicted correctly! {current_player.name}'s turn will be skipped next round.")
            current_player.skip_turn = True
            correct_opponents[0].tokens += 1
            
            # Make sure UI shows this immediately
        ...
        if len(correct_opponents) >= 2:
            self.add_message(f"{correct_opponents[1].name} predicted correctly! {current_player.name} moves backward by {dice_roll} spaces.")
        ...
        for opponent in correct_opponents[2:]:
            opponent.tokens += 1
            self.add_message(f"{opponent.name} predicted correctly and gained a token!")
    ...
    def process_game_turn(self):
        ...
        if is_human:
            ...
            # Bonus roll will be handled after the animation completes
            if bonus_roll:
        ...
        while running:
            ...
            while running and not game_over:
                ...
                if self.animation_in_progress:
                    game_over = self.update_animations()
                ...
                # If not waiting for human input or animation, proceed with game logic
                            ...
                            self.waiting_for_token_decision]):
                    # Process game turn
    ...
    def reset_game(self):
        ...
        self.game = SnakesAndLadders(
            board_size=self.game.board_size,
            num_snakes=len(self.game.snakes),
            num_ladders=len(self.game.ladders)
        )
        
        # Reset player list with the same player types
        old_players = self.game.players
        self.game.players = []
        
        for old_player in old_players:
            if isinstance(old_player, AIPlayer):
                new_player = AIPlayer(old_player.name, old_player.difficulty)
                self.game.add_player(new_player)
            elif isinstance(old_player, HumanGUIPlayer):
                new_player = HumanGUIPlayer(old_player.name, self)
                self.game.add_player(new_player)
            else:
                new_player = Player(old_player.name)
                self.game.add_player(new_player)
...
class HumanGUIPlayer(Player):
    ...
    def __deepcopy__(self, memo):
        ...
        player_copy = Player(self.name)
        player_copy.position = self.position
        player_copy.tokens = self.tokens
        player_copy.skip_turn = self.skip_turn
        return player_copy
...
# If this module is run directly, start the game
if __name__ == "__main__":
    ...
    # Create the game with a smaller board for faster gameplay
    game = SnakesAndLadders(board_size=100, num_snakes=8, num_ladders=8)
    
    # Set player colors based on user selection
    all_colors = [RED, BLUE, GREEN, PURPLE]
    remaining_colors = [c for c in all_colors if c != selected_color_value][:3]  # Get 3 other colors
    ...
    game.add_player(AIPlayer("AI Medium", difficulty="medium"))
    game.add_player(AIPlayer("AI Hard", difficulty="hard"))
